...
fid=H5F.open('/rds/projects/w/wangzt-hifi-cfd/data/JER_round/ldt/ldt-01.h5');
dset_name='ldt';
...
% load mean flow file
fprintf('%s \n','loading mean flows...');
load('/rds/projects/w/wangzt-hifi-cfd/data/JER_round/new_mesh/meanflows_tp.mat','flows');
...
for im=1:length(modeM)
    matfilename=['~/AWC/SPOD-sound/lighthill-modes-pure/azimodes/',...
...
for ifile=ist_file:ied_file
    ...
    % set hdf file name
    filename=['/rds/projects/w/wangzt-hifi-cfd/data/JER_round/ldt/' sprintf('%s%02d%s','ldt-',ifile,'.h5')];
