function [f] = create_dft_blocks(X,varargin)
%CREATES DFT BLOCKS. TAKEN FROM PART OF THE SPOD CODE. 
...
%
%   [L,P,F] = SPOD(X,WINDOW,NOVERLAP) increases the number of
...
%
%   [L,P,F] = SPOD(X,WINDOW,NOVERLAP,DT) uses the time step DT
...
%
%   [L,P,F] = SPOD(X,WINDOW,NOVERLAP,DT,OPTS) specifies options:
...
[window,nOvlp,dt,nDFT,nBlks] = spod_parser(nt,nx,isrealx,varargin{:});
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [window,nOvlp,dt,nDFT,nBlks] = spod_parser(nt,nx,isrealx,varargin)
