function H = lighthill_time_integrator_2(X,theta,mup,opts,varargin)
    ...
    % opts.xs: 1D vector of dimensionless axial grid points of shape (nx,1)
    % opts.rs: 1D vector of dimensionless radial gridpoints of shape (1,nr)
    ...
    % OUTPUTS -----------------------------------------------------------------
    % H:     1D vector, the double time derivative of which gives the far-field
    %        pressure timeseries. The length of H depends on X, theta and the
    %        largest axial and radial grid points.
    ...
    [xg,rg] = ndgrid(opts.xs,opts.rs);


    % Calculate indices of the limits of the pressure timeseries
    t0_max  = X/c0;
    t0_min  = (X/opts.D - xf*cos(theta_r) - rf*sin(theta_r))/(c0/opts.D);
    t_aug   = opts.ts;
    ...
    for m = 0:mup
        ...
        for l = 1:na
            ...
            a_l = opts.as(l);
            t_ob = (X/opts.D - xg*cos(theta_r) - ...
                        rg*sin(theta_r)*cos(a_l))/(c0/opts.D); % (nx,nr)
            t_ob = transpose(t_ob(:)); % (1,ncol)
            % Query times ti - tobs
            tq = tseg - t_ob; % (k,ncol)
            ...
            J(:,:,:,1)  = ((cos(theta_r))^2)*J(:,:,:,1);
            J(:,:,:,2)  = 2*cos(theta_r)*sin(theta_r)*cos(a_l)*J(:,:,:,2);
            J(:,:,:,3)  = -2*cos(theta_r)*sin(theta_r)*sin(a_l)*J(:,:,:,3);
            J(:,:,:,4)  = ((sin(theta_r))^2)*((cos(a_l))^2)*J(:,:,:,4);
            J(:,:,:,5)  = -((sin(theta_r))^2)*sin(2*a_l)*J(:,:,:,5);
            J(:,:,:,6)  = ((sin(theta_r))^2)*((sin(a_l))^2)*J(:,:,:,6);

            % Account for azimuthal variation of nonzero azimuthal modes
            if m > 0
                J = 2*real(exp(1i*m*a_l)*J); % (k,nx,nr,6)
            end

            S = S + squeeze(sum(permute(weight,[3,1,2]).*J,[2,3,4]));
